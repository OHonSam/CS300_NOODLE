  ...
  return (
    <div>
      ...
      <div
        className={`${
          isOpen ? "w-64" : "w-20"
        } bg-gray-900 text-white h-screen duration-300 flex flex-col relative`}
      >
        {/* Logo */}
        <div className={`flex items-center gap-4 p-4 ${isOpen? 'justify-between' : 'justify-center'}`}>
          {isOpen && (
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-xl font-medium">Noodle</h1>
            </div>
          )}
        ...
        {/* Footer */}
        <div className={`p-4 border-t border-gray-800 flex items-center gap-4 ${!isOpen && 'justify-center'}`}>
          <img
            src="https://via.placeholder.com/40"
            alt="User Profile"
            className="h-10 w-10 rounded-full"
          />
          {isOpen && <span>Hi, John</span>}
        </div>
      </div>
    </div>
  );
