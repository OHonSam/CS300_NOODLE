      ...
      <DialogBackdrop className="absolute w-[700px] bg-white px-10 py-8 z-50 focus:outline-none shadow-lg -inset-12 m-auto max-h-max rounded-xl">
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center space-y-4">
            <h4 className="text-xl font-semibold">Please wait while enrolling students...</h4>
            ...
            >
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                ...
                <p className="text-xs text-gray-500">
                  {file ? `Uploaded File: ${file.name}` : `Allowed Format: ${fileFormat.join(', ')}`}
                </p>
                {errorMessage && (
                  <p className="mt-2 text-xs text-center text-error-500">{errorMessage}</p>
                )}
              </div>
              <input 
                id="dropzone-file" 
...
export default EnrolledStudentUploadDialog;