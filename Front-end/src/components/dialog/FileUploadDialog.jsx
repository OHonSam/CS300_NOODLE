...
const FileUploadDialog = ({ heading, isOpen, onClose, onSubmit, fileFormat, userType }) => {
  ...
  const { addStudentFromFile } = useStudentInfo();
  const { addTeacherFromFile } = useTeacherInfo();
  ...
  // Get the appropriate upload function based on user type
  const getUploadFunction = () => {
    switch (userType) {
      case 'admin':
        return addAdminFromFile;
      case 'student':
        return addStudentFromFile;
      case 'teacher':
        return addTeacherFromFile;
      default:
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const message = await uploadFunction(file);
      handleClose();
